...
let ORIG_MOV = "original_mov"
let CROPPED_MOV = "cropped_mov"
...
//                        self.cameraButton.enabled = true
        ...
        func getMin(height: CGFloat, width:CGFloat) -> CGFloat {
            if height < width {
                return height
            }
            return width
        }
        typealias CropMovieComplectionBlock = (NSURL) -> Void
    
        func cropToSquareMovie(url: NSURL!, completionBlock: CropMovieComplectionBlock) {
            // Crop the input movie to a square movie by the shorter length
...
//            var outputURL = NSFileManager.defaultManager()
//                .URLForDirectory(
...
//                .URLByAppendingPathExtension( UTTypeCopyPreferredTagWithClass( AVFileTypeQuickTimeMovie , kUTTagClassFilenameExtension).takeRetainedValue() )
            ...
            var outputFilePath: String = NSTemporaryDirectory().stringByAppendingPathComponent( CROPPED_MOV.stringByAppendingPathExtension("mov")!)
            ...
            var compositionTrack = composition.addMutableTrackWithMediaType(AVMediaTypeVideo, preferredTrackID: Int32(kCMPersistentTrackID_Invalid) )
            
            // input clip
            var clipVideoTrack = asset.tracksWithMediaType(AVMediaTypeVideo)[0] as AVAssetTrack
            let squareSize = self.getMin(clipVideoTrack.naturalSize.height, width: clipVideoTrack.naturalSize.width)
...
//            videoComposition.renderScale = 1.0 
            ...
            // 'instructions' can modify the background color of your video and apply 'layer instructions'
            var instruction = AVMutableVideoCompositionInstruction()
            instruction.timeRange = CMTimeRangeMake(kCMTimeZero, clipVideoTrack.timeRange.duration)
            
            
            // 'layer instructions' can be used to apply transforms, transform ramps, opacity and opacity ramps
            var layerInstruction = AVMutableVideoCompositionLayerInstruction(assetTrack: clipVideoTrack)
            
            var t1 = CGAffineTransformMakeTranslation(squareSize, 0)
            var t2 = CGAffineTransformRotate(t1, CGFloat(M_PI_2) )
            ...
            exporter.exportAsynchronouslyWithCompletionHandler({
                if (exporter.status == AVAssetExportSessionStatus.Completed) {
                    ...
                    println("croped movie: \(exporter.outputURL) " )
                    ...
                    if (assetLibrary.videoAtPathIsCompatibleWithSavedPhotosAlbum(exporter.outputURL) ){
                        assetLibrary.writeVideoAtPathToSavedPhotosAlbum(exporter.outputURL, completionBlock:{
                            (url, error) in
                            completionBlock(exporter.outputURL)
                        })
        ...
        func captureOutput(captureOutput: AVCaptureFileOutput!, didFinishRecordingToOutputFileAtURL outputFileURL: NSURL!, fromConnections connections: [AnyObject]!, error: NSError!) {
            ...
            self.cropToSquareMovie(outputFileURL, {
                (url: NSURL!) in
        ...
        @IBAction func toggleMovieRecord(sender: AnyObject) {
            ...
            dispatch_async(self.sessionQueue, {
                if !self.movieFileOutput!.recording{
                    ...
                    var outputFilePath: String = NSTemporaryDirectory().stringByAppendingPathComponent( ORIG_MOV.stringByAppendingPathExtension("mov")!)
